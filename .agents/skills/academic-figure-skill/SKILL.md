...
When the paper has a body-page requirement, plan visual evidence before
solving so figures remain distributed across the question chapters. A late
page deficit does not authorize decorative plots. Re-open the Evidence
Matrix and prefer real-result composites such as parameter landscape plus
bootstrap, fold/device error spectra, model win/loss maps, residual strata,
feature response, boundary/threshold maps, and sensitivity intervals. Each
new panel must add evidence that is not derivable from an existing panel.
Full fold tables, file/hash checks, serialization, and delivery audits belong
in appendices or supporting material, not in a standalone figure-led body
chapter. Per-question flowcharts stay on the project Flowchart Plan and are
created only for genuinely distinct data flow, branching, iteration, or
validation logic; never generate one mechanically for every question.
