...
class LocalSubprocessBackend(ExecutionBackend):
    ...
    def execute(self, argv, *, cwd, environment, timeout, stdout, stderr):
        ...
        with stdout.open("xb") as out, stderr.open("xb") as err:
            try:
                # Record the owner's identity before launch; a missing child record
                # after interruption is deliberately not treated as proof of exit.
            ...
            try:
                child_identity = identity(process.pid)
            except psutil.NoSuchProcess:
                child_identity = {"pid": process.pid, "create_time": None}
            observation = {"owner": identity(), "child": child_identity,
                           "observed_at": now(), "argv": argv, "descendants": []}
            try:
                write_json(stdout.parent / "process.json", observation, exclusive=True)
            except (OSError, ValueError) as exc:
                if process.poll() is None:
                    process.kill()
                process.wait(timeout=10)
                return ExecutionResult(process.returncode, False, time.monotonic() - started,
                                       "Could not persist process observation: " + type(exc).__name__)
            try:
                observed = set()
                ...
                while True:
                    try:
                        ...
                        for descendant in descendants:
                            record = identity(descendant.pid)
                            key = (record["pid"], record["create_time"])
                            if key not in observed:
                                observed.add(key)
                                observation["descendants"].append(record)
                                changed = True
                        if changed:
                            write_json(stdout.parent / "process.json", observation)
                    except (psutil.NoSuchProcess, psutil.AccessDenied):
                        pass  # Capability remains best_effort, never complete OS isolation.
