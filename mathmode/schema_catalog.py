    ...
    return {
        ...
        "model_spec": contract({"question_id": ID, "method_id": ID, "decision_id": ID,
            ...
            "validation_plan": obj({"checks": arr(ID, 1, True), "independent_entrypoint": PATH,
                ...
                "assumptions": PIN,
                "screening_card": obj({"path": PATH, "sha256": HASH})}, optional=("criteria", "probe", "screening_card", "assumptions")),
        ...
        "run_manifest": contract({"run_id": ID, "question_id": ID, "method_id": ID,
            ...
            "contract_snapshots": arr(obj({"contract": enum("validation_criteria", "risk_probe_plan", "method_card", "risk_probe", "run_manifest", "assumption_plan"),
        ...
        "freeze_request": contract({"actor_id": ID, "question_id": ID, "decision_id": ID,
            ...
            "assumption_reports": arr(PIN, 1, True),
            "numbers": arr(obj({"frozen_number_id": ID, "claim_id": ID, "source_path": PATH,
                "locator": TEXT, "unit": TEXT, "precision": {"type": "integer", "minimum": 0, "maximum": 16}}), 1)}, optional=("qualification_sources", "assumption_reports")),
        "frozen_numbers": contract({"freeze_id": ID, "version": {"type": "integer", "minimum": 1},
            ...
            "registry_artifact_id": ID, "qualifications": QUALIFICATIONS,
            "assumption_reports": arr(PIN, 1, True)}, optional=("qualifications", "assumption_reports")),
        ...
        "workflow_plan": contract({"case_id": ID, "policy_path": nullable(PATH), "agent_schedule": nullable(PATH),
            ...
            "questions": arr(obj({"question_id": ID, "method_card": PATH, "probe_reports": arr(PATH, 1, True), "probe_specs": arr(PATH, 1, True),
                ...
                "baseline_code_review": nullable(PATH), "validation_review": PATH, "assumption_plan": PATH,
        ...
        "code_repair_request": contract({"repair_id": ID, "question_id": ID, "created_at": TIME,
            "diagnosis": PIN, "failed_run": PIN, "original_spec": PIN,
            "execution_role": enum("main", "baseline", "fallback", "probe"),
            "candidate_spec": PATH, "review": PATH,
            "code": arr(obj({"source": PATH, "snapshot": PIN, "candidate": PATH}), 1)}),
        "repair_activation": contract({"activation_id": ID, "question_id": ID, "created_at": TIME,
            "request": PIN, "review": PIN, "candidate_spec": PIN, "run": PIN,
            "failed_predecessor": PIN, "execution_role": TEXT, "status": enum("ACTIVE")}),
        "failure_diagnosis": contract({"diagnosis_id": ID, "actor_id": ID, "question_id": ID,
            "failure_source": obj({"path": {"type": "string", "pattern": r"^runs/[A-Za-z][A-Za-z0-9_.-]{0,95}/run_manifest\.json$"}, "sha256": HASH}), "created_at": TIME,
            "failure_class": enum("ENV_FAILURE", "DATA_FAILURE", "CODE_FAILURE", "MODEL_FAILURE", "VALIDATION_FAILURE", "POLICY_FAILURE"),
            "cause_id": ID, "rationale": TEXT, "evidence_refs": arr(ID, 1, True),
            "repair_steps": arr(TEXT, 1)}),
        ...
        "assumption_plan": contract({"plan_id": ID, "question_id": ID, "actor_id": ID,
            "created_at": TIME, "ledger": PIN, "models": obj({"main": PIN, "baseline": PIN}),
            "assumptions": arr(obj({"assumption_id": ID, "mode": enum("sensitivity", "not_applicable"),
                "rationale": TEXT, "scenario_ids": arr(ID, 0, True)}), 1),
            "scenarios": arr(obj({"scenario_id": ID, "field": enum("parameter", "seed"),
                "parameter": nullable(TEXT), "value": NUMBER, "rationale": TEXT,
                "metrics": arr(obj({"metric": ID, "unit": TEXT,
                    "max_absolute_change": {"type": "number", "minimum": 0}}), 1)}))}),
        "assumption_report": contract({"question_id": ID, "plan": PIN, "created_at": TIME,
            "producer": {"const": "assumption-assessment-service"}, "status": enum("PASS", "FAIL"),
            "scope": {"const": "declared_assumption_checks"},
            "experiments": arr(obj({"scenario_id": ID, "main_run": PIN, "baseline_run": PIN, "validation": PIN})),
            "checks": arr(obj({"scenario_id": ID, "metric": ID, "unit": TEXT, "control": NUMBER, "perturbed": NUMBER,
                "absolute_change": NUMBER, "max_absolute_change": NUMBER, "status": enum("PASS", "FAIL")})),
            "assessments": arr(obj({"assumption_id": ID, "status": enum("TESTED", "NOT_APPLICABLE", "FAILED"),
                "rationale": TEXT, "scenario_ids": arr(ID, 0, True)}), 1), "blockers": arr(TEXT)}),
        ...
        "human_decision_request": contract({"request_id": ID, "case_id": ID, "question_id": ID,
            "created_at": TIME, "task_signature": HASH,
            "task": obj({"path": PATH, "sha256": HASH}), "display": obj({"path": PATH, "sha256": HASH}),
            "card_path": PATH, "probe_paths": arr(PATH, 1, unique=True), "decision_path": PATH,
            "previous_ledger_sha256": nullable(HASH),
            "choices": arr(obj({"role": enum("main", "fallback"), "method_id": ID}), 1)}),
        "human_decision_event": contract({"event_id": ID, "request": obj({"path": PATH, "sha256": HASH}),
            "actor_id": ID, "received_at": TIME, "choice": enum("main", "fallback", "defer"), "rationale": TEXT,
            "source": {"const": "terminal_stdin"}, "input_tty": {"const": True}, "output_tty": {"const": True},
            "confirmation": {"const": "SUBMIT"}, "identity_scope": {"const": "host_terminal_not_person_authentication"}}),
